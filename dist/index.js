...
class NinjaOneMCPServer {
        ...
        try {
            ...
            switch (name) {
                ...
                case 'create_organization':
                    data = await this.api.createOrganization(args.name, args.description, args.nodeApprovalMode, args.tags);
                    break;
                case 'update_organization':
                    data = await this.api.updateOrganization(args.id, args.name, args.description, undefined, args.tags);
                    break;
                // Location CRUD
                case 'create_location':
                    data = await this.api.createLocation(args.organizationId, args.name, args.address, args.description);
                    break;
                case 'update_location':
                    data = await this.api.updateLocation(args.organizationId, args.locationId, args.name, args.address, args.description);
                    break;
            ...
            throw new McpError(ErrorCode.InternalError, `API call failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async callAPIMethod(name, args) {
        switch (name) {
            // Device Management
            case 'get_devices':
            ...
            case 'set_device_maintenance': {
                ...
                if (!permanent) {
                    ...
                    durationUnit = providedUnit;
                    const durationValueNumber = args.durationValue;
                    durationValue = durationValueNumber;
                ...
                if (args.disabledFeatures !== undefined) {
                    if (!Array.isArray(args.disabledFeatures) || args.disabledFeatures.some((item) => typeof item !== 'string')) {
                ...
                if (durationUnit) {
                    maintenanceOptions.durationUnit = durationUnit;
                }
                if (typeof durationValue === 'number') {
                    maintenanceOptions.durationValue = durationValue;
                }
                if (typeof durationSeconds === 'number') {
                    maintenanceOptions.durationSeconds = durationSeconds;
                }
            ...
            case 'set_region':
                if (args.baseUrl)
                    this.api.setBaseUrl(args.baseUrl);
                else if (args.region)
                    this.api.setRegion(args.region);
                else
                    throw new McpError(ErrorCode.InvalidParams, 'Provide either region or baseUrl');
                return { ok: true, baseUrl: this.api['baseUrl'] };
            // Device Control
            ...
            case 'update_end_user':
                return this.api.updateEndUser(args.id, args.firstName, args.lastName, args.email, args.phone);
    ...
    async searchDevicesByName(searchName, limit) {
        const devices = await this.api.getDevices(undefined, 200);
        const filtered = devices
            .filter((device) => device.systemName?.toLowerCase().includes(searchName.toLowerCase()) ||
    ...
    async findWindows11Devices(limit) {
        const devices = await this.api.getDevices(undefined, 200);
        const windowsDevices = devices.filter((device) => device.nodeClass === 'WINDOWS_WORKSTATION' || device.nodeClass === 'WINDOWS_SERVER');
        ...
        for (const device of windowsDevices.slice(0, 50)) {
            try {
                ...
                if (details.os?.name?.includes('Windows 11')) {
                    ...
                    if (windows11Devices.length >= limit)
                        break;
                }
            }
            catch (error) {
